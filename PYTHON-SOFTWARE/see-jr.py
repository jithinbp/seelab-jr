# -*- coding: utf-8 -*-
# see-jr - software stack to support the seelablet-junior.
#
...
class Interface():
	...
	* loads calibration values.
	...
	>>> I = see-jr.open()
	>>> self.__print__(I)
	<see-jr.Interface instance at 0xb6c0cac>
	...
	def __runInitSequence__(self,**kwargs):
		...
		self.digital_channel_names=['ID1','OD1','PDC1','PDC2','SEN','SQR','CNTR']
	...
	def capture2(self,ns,tg,TraceOneRemap='A1'):
		"""
	...
	def capture4(self,ns,tg,TraceOneRemap='A1'):
		"""
	...
	def capture_hr(self,chan,samples,tg,*args):
		try:
	...
	def capture_hr_multiple(self,samples,tg,*args):
		...
		**Arguments** 
		...
		try:
			...
			self.H.__sendByte__(len(args)-1)  #channels to read . max 4
			self.H.__sendInt__(CHANNEL_SELECTION|((total_chans-1)<<12) )
			self.H.__sendInt__(samples)        		#number of samples to read
			self.H.__sendInt__(int(self.timebase*8))        #Timegap between samples.  8MHz timer clock
			self.H.__get_ack__()
			time.sleep(1e-6*samples*self.timebase*len(args)+.01)
	...
	def set_state(self,**kwargs):
		...
		**Arguments** 
		...
		if 'OD2' in kwargs:
			data|= 0x80|(8 if kwargs.get('OD2') else 0)
		try:
			print (hex(data))
	...
	def __capture_capacitance__(self,samples,tg):
		...
		try:
			x,y=self.capture_hr('CAP',samples,tg,'READ_CAP')
		except Exception as ex:
			self.raiseException(ex, "Communication Error , Function : "+inspect.currentframe().f_code.co_name)
		try:
...
if __name__ == "__main__":
	...
	#plot(x,I.DAC.CHANS['PV1'].polynomial(x)-x)
	#show()
